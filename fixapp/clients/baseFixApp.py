...
class BaseFixClient(fix.Application):
    ...
    def onCreate(self, sessionID):
        ...
        '''
        printvvv(self.settingsDic)
        return

    def onLogon(self, sessionID):
        self.sessionID = sessionID
        #self.settingsDic = self.session_settings.get(sessionID)
        printv ("Successful Logon to session '{}'.".format(sessionID.toString()))
        return

    def onLogout(self, sessionID):
        printv("Logging out from session: {}".format(sessionID.toString()))
    ...
    def toAdmin(self, message,sessionID):
        ...
        printvvv([sess.toString() for sess in self.session_ids])
        #self.settingsDic = self.session_settings.get(sessionID)
        ...
        if msg_type == fix.MsgType_Logon:
            printvvv("Message is type 'Logon' or {}".format(fix.MsgType_Logon))
    ...
    def _make_standard_header(self,sess_type):
        ...
        *     52 - SendingTime  - required
        ...
        printvvv("Standard Header: " + fix_str)
        return msg
